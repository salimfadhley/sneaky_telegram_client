...
def store(update: UpdateNewChannelMessage, user:User, channel:Channel) -> None:
    ...
    update_id = hash((user.id, channel.id, update.message.id))

    es.index(index="updates", doc_type="update", id=update_id, body=update_as_dict)
    es.index(index="channels", doc_type="channel", id=channel.id, body=channel.to_dict())
